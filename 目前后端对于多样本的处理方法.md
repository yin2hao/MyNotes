...
| <span style="background-color: yellow">核验（确定）</span>     | 发送`gasData/auditConfirm` | 更新task，commission</br>对当前任务的测定数据的结果进行处理，在`t_gas_data`中存储最终结果(合格与否)，并创建三个证书</br>**具体修改逻辑待定**                                                                                                                                        | 不重要                                                                       |
| <span style="background-color: red">下载文件(分页查询)</span>    | 发送`report/queryPage`     | 根据传入数据查询对应的`t_gas_report`表单项并分页返回前端</br>预计的改动项与“测试任务（分页查询）”相似（目前未修改）                                                                                                                                                               | 抛开响应信息（比如请求是否成功什么的）不说，主体信息（即查询的信息）是一个二维数组`List<List<gasReportQueryForm>>` |
|                                                          |                          |                                                                                                                                                                                                                                    |                                                                           |
